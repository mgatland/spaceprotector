...
var Level = function(mapData, tileSize) {
	...
	var loadMap = function (mapData) {
		...
		while (mapData[n]) {
			if (mapData[n]==="m") {
				Events.monster(Monsters.create1(level, x*tileSize, y*tileSize));
				map[y][x] = 0;
			}
			if (mapData[n]==="k") {
				Events.monster(Monsters.create2(level, x*tileSize, y*tileSize));
				map[y][x] = 0;
			}
			if (mapData[n]==="x") {
				Events.monster(Monsters.createCrate(level, x*tileSize, y*tileSize));
				map[y][x] = 0;
			}
			if (mapData[n]==="!") {
